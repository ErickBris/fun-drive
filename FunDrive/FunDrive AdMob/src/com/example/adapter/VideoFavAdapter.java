...
public class VideoFavAdapter extends ArrayAdapter<VideoPojo> {

	private Activity activity;
	private List<VideoPojo> itemsLatestring;
	private VideoPojo objLatestBeanring;
	private int row;
	...
	public VideoFavAdapter(Activity act, int resource, List<VideoPojo> arrayList) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.textvname.setText(objLatestBeanring.getFVItemName().toString());
		...
		if(objLatestBeanring.getFVItemPlayIds().equals("000q1w2"))
		{
			imageLoader.DisplayImage(Constant.SERVER_IMAGE_UPFOLDER_THUMB+objLatestBeanring.getFVItemThumb().toString(), holder.imgv_latetst);
		}
		else
		{
			imageLoader.DisplayImage(Constant.YOUTUBE_IMAGE_FRONT+objLatestBeanring.getFVItemPlayIds().toString()+Constant.YOUTUBE_IMAGE_BACK, holder.imgv_latetst);
		}
